...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT crossref-local's —
...
# so only crossref-local's own thin dep set is fetched per run.
#
...
export TMPDIR="/tmp/ci-crossref_local-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
rm -rf "$TMPDIR"
...
# crossref-local's reproducibility tests (validate_recipe renders the SAME recipe
...
# Install crossref-local + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors crossref-local's historical bare-uv/pip workflow so a
...
# crossref-local's suite is ~2460 tests; single-process it overran the job's old
...
# crossref-local style-stack are naturally isolated per worker — the safe way to
# parallelise a matplotlib-heavy suite.
#
# Worker count: use ALL cores. Each matrix leg now runs on its own dedicated
# self-hosted node (one runner per node: crossref-local-01/02/03), so there is no
...
# loadscope pins an entire MODULE's tests to ONE worker — and crossref-local's heavy
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/crossref_local --cov-report=xml --cov-report=term \
    -p no:cacheprovider
