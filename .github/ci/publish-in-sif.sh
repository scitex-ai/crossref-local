...
# (project=crossref-local, owner=ywatanabe1989, repo=crossref-local,
...
TMPDIR="/tmp/publish-crossref_local-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
export TMPDIR
rm -rf "$TMPDIR"
...
echo "PUBLISH-OK: crossref-local dist/* uploaded to PyPI via manual OIDC trusted publishing"
